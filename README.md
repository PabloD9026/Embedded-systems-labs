...
В качестве микроконтроллеров были использованы микроконтроллеры на базе STM32F103C8T6 под названием Bluepill. Так как с этими микроконтроллерами нет возможности создать хост для USB, а только устройство, нужно было использовать компьютер в качестве хоста. Компьютер только передает данные с одного микроконтроллера в другой, он данные не меняет. Программа на компьютере можно посмотреть в файлах. Она написанна на python, она принимает данные из платы, расшифрует их на char, преобразует из на string, и отправляет их на другую плату.

Микроконтроллер, который отправляет данные и рассчитает и отправляет данные, дальше будет обозначаться как «Тх микроконтроллер» а микроконтроллер, который принимает данные будет «Rx микроконтроллер». Сначала был настроен Тх микроконтроллер. Это показано ниже

Распиновка

![image](https://github.com/PabloD9026/Embedded-systems-labs/assets/91127156/a8ff068c-5905-4d02-9f65-e5bab14d3030)

настройки 

![image](https://github.com/PabloD9026/Embedded-systems-labs/assets/91127156/21a94860-c31f-45ba-b0cd-7ddd67e60763)

![image](https://github.com/PabloD9026/Embedded-systems-labs/assets/91127156/5cac6c6e-a790-47a4-8893-c0a68e59f155)

Стоит отметить, что все настройки для USB были оставлены по дефолту. 

Настройки Rх микроконтроллер для USB идентичные на настройки для USB, поэтому настройки показаны ниже только распиновка, и SPI, который нужен для того, чтобы использовать светодиодную матрицу.

![image](https://github.com/PabloD9026/Embedded-systems-labs/assets/91127156/54dec799-9997-49c8-a5a2-643a7b0ecd77)

![image](https://github.com/PabloD9026/Embedded-systems-labs/assets/91127156/6196a19e-bf3f-4555-b1fb-6fd0a4d198f9)
